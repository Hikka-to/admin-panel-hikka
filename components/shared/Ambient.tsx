...
const Ambient: React.FC<AmbientProps> = ({
  children,
  ambientSize = AmbientSize.lg,
}) => {
  const blur =
    BlurSize[
      Object.keys(AmbientSize).find(
        (k) => AmbientSize[k as keyof typeof AmbientSize] == ambientSize,
      ) as keyof typeof BlurSize
    ];

  const contentRef = useRef<HTMLDivElement>(null);
  const firstCanvas = useRef<HTMLCanvasElement>(null);
  const secondCanvas = useRef<HTMLCanvasElement>(null);
  const [selectedCanvas, setSelectedCanvas] = React.useState<CanvasType>(
    CanvasType.First,
  );
  const [transitionedCanvas, setTransitionedCanvas] =
    React.useState<CanvasType>(CanvasType.First);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (
        ![...content.querySelectorAll("*")].find(
          (e) => e == document.fullscreenElement,
        )
      ) {
        const canvas =
          selectedCanvas === CanvasType.First ? secondCanvas : firstCanvas;
      ...
      setSelectedCanvas(
        selectedCanvas === CanvasType.First
          ? CanvasType.Second
          : CanvasType.First,
      );
    })();
  ...
  useEffect(() => {
    const currentCanvas =
      selectedCanvas === CanvasType.First ? firstCanvas : secondCanvas;
    const transitionedHandler = () => {
      setTransitionedCanvas(selectedCanvas);
      currentCanvas.current?.removeEventListener(
        "transitionend",
        transitionedHandler,
      );
    };

    currentCanvas.current?.addEventListener(
      "transitionend",
      transitionedHandler,
    );
  }, [selectedCanvas]);
